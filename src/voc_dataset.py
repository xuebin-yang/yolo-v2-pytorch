...
class VOCDataset(Dataset):
    def __init__(self, root_path="/home/xufanglei/datasets/VOC", year="2007", mode="train", image_size=448, is_training = True):
    ...
    def __getitem__(self, item):
        ...
        for obj in annot.findall('object'):
            xmin, xmax, ymin, ymax = [int(obj.find('bndbox').find(tag).text) - 1 for tag in
                                      ["xmin", "xmax", "ymin", "ymax"]]    # 获取真实bbox的左上角和右下角坐标
            label = self.classes.index(obj.find('name').text.lower().strip())
            objects.append([xmin, ymin, xmax, ymax, label])  # object 就是真实bbox
        if self.is_training:
