...
class Crop(object):
    ...
    def __call__(self, data):
        ...
        label = [[lb[0] - new_xmin, lb[1] - new_ymin, lb[2] - new_xmin, lb[3] - new_ymin, lb[4]] for lb in label]   # 标签也会跟着变
...
class HSVAdjust(object):
    ...
    def __call__(self, data):
        ...
        image, label = data
        adjust_hue = uniform(-self.hue, self.hue) # [-30, 30]
        ...
        image = image.astype(np.float32) / 255   # 图像做归一化
        ...
        image = (image * 255).astype(np.float32)   # 图像恢复
