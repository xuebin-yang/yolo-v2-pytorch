...
class Yolo(nn.Module):
    def __init__(self, num_classes,
                 anchors=[(1.3221, 1.73145), (3.19275, 4.00944), (5.05587, 8.09892), (9.47112, 4.84053),
                          (11.2364, 10.0071)]):  # yolo v2 用了 5 个 anchor base 而且只给了宽高
    ...
    def forward(self, input):
        output = self.stage1_conv1(input)  # [10, 32, 224, 224]
        output = self.stage1_conv2(output)  # [10, 64, 112, 112]
        output = self.stage1_conv3(output)  # [10, 128, 112, 112]
        ...
        output = self.stage1_conv13(output)  # [2, 512, 28, 28]
        ...
        output_1 = self.stage2_a_conv7(output_1)  # [2, 1024, 14, 14]
        print('output_1.shape', output_1.shape)

        output_2 = self.stage2_b_conv(residual)  # [2, 64, 28, 28]
        # print('output_2.shape', output_2.shape)
        batch_size, num_channel, height, width = output_2.data.size()
        output_2 = output_2.view(batch_size, int(num_channel / 4), height, 2, width, 2).contiguous()  # [10, 16, 28, 2, 28, 2]
        output_2 = output_2.permute(0, 3, 5, 1, 2, 4).contiguous()  # [10, 2, 2, 16, 28, 28]
        output_2 = output_2.view(batch_size, -1, int(height / 2), int(width / 2))  # [10, 256, 14, 14]

        output = torch.cat((output_1, output_2), 1)  # [2, 1280, 14, 14]
        print('output_cat.shape', output.shape)
        output = self.stage3_conv1(output)
        output = self.stage3_conv2(output)  # [2, 125, 14, 14]
        print('output_3.shape', output.shape)
