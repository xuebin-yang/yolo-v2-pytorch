...
def get_args():
    ...
    parser.add_argument("--batch_size", type=int, default=2, help="The number of images per batch")
    ...
    parser.add_argument("--es_patience", type=int, default=0,  # 一定 epochs 精度没有提高就会自动停止训练， es_min_delta就是这个精度
    ...
    parser.add_argument("--data_path", type=str, default="/home/xufanglei/datasets/", help="the root folder of dataset")
...
def train(opt):
    ...
    if os.path.isdir(log_path):   # 判断对象是否为一个目录
        shutil.rmtree(log_path)  # 删除 log_path
    os.makedirs(log_path)   # 创建 log_path 路径
    ...
    criterion = YoloLoss(training_set.num_classes, model.anchors, opt.reduction)   # YOLO下采样层下降32倍得到13x13的特征图
    optimizer = torch.optim.SGD(model.parameters(), lr=1e-5, momentum=opt.momentum, weight_decay=opt.decay)
    best_loss = 1e10
    best_epoch = 0
    model.train()
    num_iter_per_epoch = len(training_generator)  # 一个 epoch 里有多少个 batch
    for epoch in range(opt.num_epoches):
        if str(epoch) in learning_rate_schedule.keys():
            for param_group in optimizer.param_groups:
                param_group['lr'] = learning_rate_schedule[str(epoch)]
        for iter, batch in enumerate(training_generator):
            image, label = batch   # 这里的图片是没有经过归一化的， 标签也是没有经过归一化的，标签代表的都是绝对坐标
            print('image.shape', image.shape)  # [2, 3, 448, 448]
            ...
            optimizer.zero_grad()
            logits = model(image)  #
            print('logits.shape', logits.shape)   # [2, 125, 14, 14]
